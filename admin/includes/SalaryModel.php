...
class SalaryModel
{
    ...
    {
        // Create a query that combines various types of activities
        $query = "
            (SELECT 
                ...
                s.status as status,
                CASE 
                    WHEN s.status = 'paid' THEN 'bg-green-100 text-green-800'
                    WHEN s.status = 'pending' THEN 'bg-yellow-100 text-yellow-800' 
            ...
            (SELECT 
                'Advance Request' as type,
                u.name as user,
                a.request_date as date,
                a.status as status,
                CASE 
                    WHEN a.status = 'pending' THEN 'bg-yellow-100 text-yellow-800'
                    WHEN a.status = 'approved' THEN 'bg-blue-100 text-blue-800'
                    WHEN a.status = 'rejected' THEN 'bg-red-100 text-red-800'
                    WHEN a.status = 'paid' THEN 'bg-green-100 text-green-800'
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Format the status to capitalize first letter
    ...
    {
        $query = "SELECT COUNT(*) as pending_count FROM tasks WHERE status = 'pending'";
        $result = $this->conn->query($query);
        $row = $result->fetch_assoc();

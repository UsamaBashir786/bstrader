...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($email_err) && empty($phone_err) && empty($address_err) && 
       ...
       empty($password_err) && empty($confirm_password_err) && empty($pic_err)){
        ...
        if($stmt = $conn->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ssssssssss", $param_name, $param_email, $param_phone, $param_address, 
                            $param_cnic, $param_contract_start, $param_contract_end, $param_profile_pic, 
                            $param_password, $param_role);
...
<head>
    ...
    <title>BS Traders - Register</title>
